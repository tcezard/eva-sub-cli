...
def compare_names_in_files_and_samples(sample_name_in_analysis, sample_name_per_file):
    ...
    sample_names_in_vcf = set(
        sample_name
        for sample_name_list in sample_name_per_file.value()
    ...
    if more_submitted_files_metadata:
        for file_name in sample_name_per_file:
            more_per_submitted_files_metadata[file_name] = set(sample_name_per_file[file_name]) - set(sample_name_in_analysis)
        has_difference = True
...
def read_metadata_json(json_file):
    with open(json_file) as open_json:
        ...
        for sample_info in metadata['samples']:
            samples_per_analysis[sample_info.get('analysis_alias')].append(sample_info.get('name_in_vcf'))
        for file_info in metadata['files']:
            files_per_analysis[file_info.get('analysis_alias')].append(file_info.get('fileName'))
